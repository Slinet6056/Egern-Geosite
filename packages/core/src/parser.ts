...
export function parseListText(
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    try {
      ...
      throw new GeositeParseError(
        `error in ${normalizedList} at line ${lineNo}: ${message}`,
      );
    }
  }
...
export function parseListsFromText(
...
function parseEntryLine(
  ...
  for (const part of parts.slice(1)) {
    if (part.startsWith("@")) {
      ...
      if (!VALID_ATTR_CHAR.test(attr)) {
        throw new GeositeParseError(
          `invalid attribute: ${JSON.stringify(attr)}`,
        );
      }
      attrs.push(attr);
      continue;
    ...
    if (part.startsWith("&")) {
      ...
      if (!VALID_SITE_NAME.test(affiliation)) {
        throw new GeositeParseError(
          `invalid affiliation: ${JSON.stringify(affiliation)}`,
        );
      }
    ...
    throw new GeositeParseError(
      `invalid attribute/affiliation: ${JSON.stringify(part)}`,
    );
  }
  ...
  if (type === "include") {
    if (affiliations.length > 0) {
      throw new GeositeParseError(
        `affiliation is not allowed for include:${JSON.stringify(value)}`,
      );
    }
    ...
    if (!VALID_SITE_NAME.test(sourceList)) {
      throw new GeositeParseError(
        `invalid include list name: ${JSON.stringify(value)}`,
      );
    }
...
function validateRegex(pattern: string): void {
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    throw new GeositeParseError(
      `invalid regexp ${JSON.stringify(pattern)}: ${message}`,
    );
  }
}
