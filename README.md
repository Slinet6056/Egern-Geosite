...
| `standard` (default) | Converts entries that remain safe as host-only URL patterns: exact domain (`^x$`), optional-subdomain suffix (`(^\|\\.)x$`), general end-anchored host suffix (`x$`, including forms like `javdb\d+\.com$`), or domain prefix (`^x`). Entries with lookaheads, backreferences, slashes, or top-level alternation are dropped |
