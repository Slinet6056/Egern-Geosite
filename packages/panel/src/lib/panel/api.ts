...
function buildPath(base: string, mode: PanelMode, name: string, filter: string | null): string {
	const withFilter = normalizeNameWithFilter(name, filter);
	return `${base}/${mode}/${encodeURIComponent(withFilter)}`;
}

export function buildRulesApiPath(mode: PanelMode, name: string, filter: string | null): string {
	return buildPath(GEOSITE_API_BASE, mode, name, filter);
}

export function buildRulesPublicPath(mode: PanelMode, name: string, filter: string | null): string {
